{ self, ... }:
{
  flake.nixosModules.kernel.imports = [
    self.nixosModules.kernel-vtless
    self.nixosModules.kernel-hardened
  ...
  flake.nixosModules.no-wall = {
    ...
    # Scheduling a poweroff will `wall` to every _registered_ terminal.
    # Only terminals that it can reach are thus SSH pseudottys. Spam.
    services.logind.settings.Login.WallMessages = false;
  };

  flake.nixosModules.kernel-vtless =
    {
      config,
      lib,
      pkgs,
    ...
    let
      ...
      inherit (lib.kernel) no option;
      inherit (lib.lists) singleton;
      inherit (lib.meta) getExe;
      inherit (lib.modules) mkForce;
      inherit (utils) escapeSystemdExecArgs;

      # TODO: libghostty powered replacement.
      kmsconify = command: {
        serviceConfig.ExecStart = [
          "" # Reset rather than append. Funny, and stupid idiom.
        ...
        serviceConfig.StandardInput = "null";
        serviceConfig.StandardOutput = "journal";
      };

    in
    {
      disabledModules = singleton "services/ttys/getty.nix";
      console.enable = false;

      systemd.services = genAttrs [ "emergency" "rescue" ] (
        mode:
        kmsconify [
          "${config.systemd.package}/lib/systemd/systemd-sulogin-shell"
          mode
        ]
      );
      ...
      boot.kernelPatches = singleton {
        ...
        structuredExtraConfig = {
          ...
          # These only exist if `VT` exists. So disable them.
          FRAMEBUFFER_CONSOLE = mkForce <| option no;
          FRAMEBUFFER_CONSOLE_DEFERRED_TAKEOVER = mkForce <| option no;
          FRAMEBUFFER_CONSOLE_DETECT_PRIMARY = mkForce <| option no;
          FRAMEBUFFER_CONSOLE_ROTATION = mkForce <| option no;

          # The framebuffer emulator that wraps DRM will always modeset,
          # removing framebuffers and their emulation kills two birds with one stone.
          FB_DEVICE = option no;
          DRM_FBDEV_EMULATION = mkForce no;
        };
      };
    };
  ...
  flake.nixosModules.kernel-hardened = {
