{ self, ... }:
{
  ...
  flake.homeModules.darwin-wm =
    ...
    {
      # SPOONS
      xdg.config.files."hammerspoon/Spoons/PaperWM.spoon".source =
        mkIf isDarwin
      ...
      xdg.config.files."hammerspoon/Spoons/Swipe.spoon".source =
        mkIf isDarwin
        <| pkgs.fetchFromGitHub {
          owner = "mogenson";
          repo = "Swipe.spoon";
      ...
      # INIT
      xdg.config.files."hammerspoon/init.lua".text =
        mkIf isDarwin
        <| mkAfter /* lua */ ''
          ---@type table
          _G.hs = _G.hs
